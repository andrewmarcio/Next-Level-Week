import { Request, Response } from 'express';
import knex from '../database/connection';

class PointsController {

    async index(req : Request, res: Response) {
        const {city, uf, items} = req.query;

        const parsedItems = String(items)
            .split(',')
            .map(item => Number(item.trim()));

        const points = await knex('points')
            .join('point_items', 'points.id', '=' ,'point_items.point_id')
            .whereIn('point_items.item_id', parsedItems)
            .where('city', String(city))
            .where('uf', String(uf))
            .distinct()
            .select('points.*');

            
        const serializedPoints = points.map(point => {
            return {
                ...points,
                img_url: `http://192.168.0.124:3333/uploads/image-points/${point.image}`
            };
        });
            
        return res.json(serializedPoints);
    }

    async show(req : Request, res: Response) {
        const {id} = req.params;
    
        const point = await knex('points').where('id', id).first();
        
        if(!point){
            return res.status(400).json({ message : "Ponto não foi encontrado!"});
        }

        const serializedPoints = {
                ...point,
                img_url: `http://192.168.0.124:3333/uploads/image-points/${point.image}`
        };

        const items = await knex('items')
            .join('point_items', 'items.id', '=' ,'point_items.item_id')
            .where('point_items.point_id', id);
        
            return res.json({
                point : serializedPoints,
                items : items
            });
    }

    async create(req: Request, res: Response) {

        const {
            name,
            email,
            whatsapp,
            latitude,
            longitude,
            city,
            uf,
            items
        } = req.body;

        const trx = await knex.transaction();

        const point = {
            image: req.file.filename,
            name,
            email,
            whatsapp,
            latitude,
            longitude,
            city,
            uf
        };

        const pointResgistred = await trx('points').insert(point);

        const point_id = pointResgistred[0];

        const pointItemRequest = items
        .split(',')
        .map((item: string) => Number(item.trim()))
        .map((item_id: number) => {
            return {
                item_id,
                point_id,
            };
        });

        const registredPointItems = await trx('point_items').insert(pointItemRequest);

        await trx.commit();

        if (registredPointItems) {
            return res.json({ status: "success", message: "Ponto de coleta cadastrado com sucesso.", data: point});
        }else{
            return res.json({ status: "error", message: "Erro ao cadastrar ponto de coleta."});
        }
    }

}

export default PointsController;